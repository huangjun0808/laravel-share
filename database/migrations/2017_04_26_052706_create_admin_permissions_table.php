...
class CreateAdminPermissionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('admin_permission', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name',125)->default('')->index()->comment('权限规则');
            $table->string('uri',125)->default('')->comment('路由地址');
            $table->string('label',125)->default('')->comment('权限解释名称');
            ...
            $table->string('icon',125)->default('')->comment('图标');
            $table->timestamps();
        });
